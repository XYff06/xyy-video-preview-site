...
/** 管理弹窗的主标签页。 */
type AdminTab = 'tag' | 'title' | 'episode';
/** 管理弹窗中的操作模式。 */
type ActionMode = 'create' | 'rename' | 'delete' | 'batch';
...
type AppState = {
  ...
  flashMessage: string;
  flashAutoCloseTimeout: number | null;
  flashVersion: number;
  flashVersionRendered: number;
  activeTagAction: Exclude<ActionMode, 'batch'>;
  activeTitleAction: Exclude<ActionMode, 'batch'>;
  activeEpisodeAction: ActionMode;
};
 ...
 */
const formatDateTime = (isoDateTime: string): string => new Date(isoDateTime).toLocaleString('zh-CN', { hour12: false });
 ...
 */
function getCurrentSeriesNameFromPath() {
 ...
 * @returns {Promise<void>}
 */
async function loadInitialSeriesData() {
  try {
  ...
  if (!getCurrentSeriesNameFromPath()) {
    await loadHomeSeriesPage();
  }
}
 ...
 * @returns {Promise<void>}
 */
async function loadHomeSeriesPage() {
  ...
  try {
    ...
    appState.homeLoading = false;
    appState.homeError = getErrorMessage(error);
  }
...
function render() {
  ...
  if (appState.adminModalOpen) {
    ...
    document.querySelectorAll('[data-admin-tab]').forEach((btn) => {
      btn.onclick = () => {
        appState.activeAdminTab = btn.dataset.adminTab as AdminTab;
        render();
      };
    });
  ...
  const activeName = getCurrentSeriesNameFromPath();
  if (activeName) {
...
function renderHome(container) {
  ...
  searchForm.onsubmit = (event) => {
    ...
    appState.sortBy = sortSelect.value as SortBy;
    appState.currentPage = 1;
    loadHomeSeriesPage();
  };
...
function renderAdminPanel(container) {
  if (appState.activeAdminTab === 'tag') {
    const tags = getAvailableTagNames();
    container.innerHTML = `
    ...
    document.querySelectorAll('[data-tag-action]').forEach((btn) => {
      btn.onclick = () => {
        appState.activeTagAction = btn.dataset.tagAction as Exclude<ActionMode, 'batch'>;
        render();
      };
    });
  ...
  if (appState.activeAdminTab === 'title') {
    ...
    document.querySelectorAll('[data-title-action]').forEach((btn) => {
      btn.onclick = () => {
        appState.activeTitleAction = btn.dataset.titleAction as Exclude<ActionMode, 'batch'>;
        render();
      };
    });
    ...
    if (titleRenameForm) {
      ...
      titleRenameForm.onsubmit = async (event) => {
        ...
        try {
          ...
          if (getCurrentSeriesNameFromPath() === oldName) history.replaceState({}, '', `/${encodeURIComponent(newName)}`);
          await loadInitialSeriesData();
        } catch (error) {
          setFlashMessage(error.message);
          render();
    ...
    if (titleDeleteForm) {
      titleDeleteForm.onsubmit = async (event) => {
        ...
        try {
          ...
          if (getCurrentSeriesNameFromPath() === oldName) history.replaceState({}, '', '/');
          await loadInitialSeriesData();
        } catch (error) {
          setFlashMessage(error.message);
          render();
    ...
    <section class="admin-panel">
      ...
      <section class="action-panel ${appState.activeEpisodeAction === 'batch' ? '' : 'hidden'}">
        <form id="episode-batch-form" class="stack-form">
          ...
          ${getTagMultiSelectHtml('batchTags', getAvailableTagNames())}
  ...
  document.querySelectorAll('[data-episode-action]').forEach((btn) => {
    btn.onclick = () => {
      appState.activeEpisodeAction = btn.dataset.episodeAction as ActionMode;
      render();
    };
  });
  ...
  if (episodeCreateForm) {
    episodeCreateForm.onsubmit = async (event) => {
      ...
      try {
        ...
        if (getCurrentSeriesNameFromPath() === payload.titleName) {
  ...
  if (episodeBatchForm) {
    ...
    episodeBatchForm.onsubmit = async (event) => {
      ...
      try {
        ...
        if (getCurrentSeriesNameFromPath() === payload.name) {
...
window.addEventListener('popappState', () => {
  if (getCurrentSeriesNameFromPath()) {
    render();
    return;
  }
