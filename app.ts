...
async function loadHomeSeriesPage() {
    ...
    if (appState.selectedTag)
        params.set('tag', appState.selectedTag);
...
function renderDetail(container, series) {
    ...
    prevEpisodePageBtn.onclick = () => {
        if (appState.episodePage <= 1)
            return;
        appState.episodePage -= 1;
        render();
    };
    nextEpisodePageBtn.onclick = () => {
        if (appState.episodePage >= totalEpisodePages)
            return;
        appState.episodePage += 1;
        render();
    };
    ...
    `;
            ...
            renameForm.onsubmit = async (event) => {
                ...
                try {
                    ...
                    if (appState.selectedTag === tag)
                        appState.selectedTag = newTagName;
                    await loadInitialSeriesData();
                }
                catch (error) {
                    setFlashMessage(error.message);
        ...
        if (deleteForm) {
            deleteForm.onsubmit = async (event) => {
                ...
                if (!tag)
                    return;
                if (!confirm(`确认删除标签“${tag}”？会从所有漫剧里移除该标签。`))
                    return;
                try {
    ...
    `;
        ...
        if (titleRenameForm) {
            ...
            titleRenameForm.onsubmit = async (event) => {
                ...
                const newTags = formData
                    ...
                    .map((tag) => String(tag).trim())
                    .filter(Boolean);
                if (!oldName || !newName || !newPoster || newTags.length === 0)
                    return;
                try {
                    ...
                    if (getCurrentSeriesNameFromPath() === oldName)
                        history.replaceState({}, '', `/${encodeURIComponent(newName)}`);
                    await loadInitialSeriesData();
                }
                catch (error) {
                    setFlashMessage(error.message);
        ...
        if (titleDeleteForm) {
            titleDeleteForm.onsubmit = async (event) => {
                ...
                if (!oldName)
                    return;
                if (!confirm(`确认删除漫剧“${oldName}”？该漫剧下全部剧集会删除。`))
                    return;
                try {
  ...
  `;
    ...
    if (episodeUpdateForm) {
        ...
        episodeUpdateForm.onsubmit = async (event) => {
            ...
            if (!payload.titleName || Number.isNaN(payload.episodeNo) || Number.isNaN(payload.newEpisodeNo))
                return;
            try {
    ...
    if (episodeDeleteForm) {
        ...
        episodeDeleteForm.onsubmit = async (event) => {
            ...
            if (!payload.titleName || Number.isNaN(payload.episodeNo))
                return;
            if (!confirm(`确认删除「${payload.titleName}」第${payload.episodeNo}集？`))
                return;
            try {
