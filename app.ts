...
async function loadHomeSeries() {
  ...
  try {
    ...
    state.homeTotal = 0;
    state.homeLoading = false;
    state.homeError = getErrorMessage(error);
  }
...
function render() {
  ...
  if (state.adminModalOpen) {
    ...
    document.querySelectorAll('[data-admin-tab]').forEach((btn) => {
      btn.onclick = () => {
        state.activeAdminTab = btn.dataset.adminTab as AdminTab;
        render();
      };
    });
...
function renderAdminPanel(container) {
  if (state.activeAdminTab === 'tag') {
    ...
    document.querySelectorAll('[data-tag-action]').forEach((btn) => {
      btn.onclick = () => {
        state.activeTagAction = btn.dataset.tagAction as Exclude<ActionMode, 'batch'>;
        render();
      };
    });
  ...
  if (state.activeAdminTab === 'title') {
    ...
    document.querySelectorAll('[data-title-action]').forEach((btn) => {
      btn.onclick = () => {
        state.activeTitleAction = btn.dataset.titleAction as Exclude<ActionMode, 'batch'>;
        render();
      };
    });
  ...
  document.querySelectorAll('[data-episode-action]').forEach((btn) => {
    btn.onclick = () => {
      state.activeEpisodeAction = btn.dataset.episodeAction as ActionMode;
      render();
    };
  });
