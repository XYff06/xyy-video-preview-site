  ...
  `;
    ...
    pagination.querySelectorAll('[data-page-no]').forEach((btn) => {
        btn.onclick = () => {
            ...
            if (!Number.isFinite(pageNo) || pageNo === state.currentPage)
                return;
...
function renderDetail(container, series) {
    ...
    prevEpisodePageBtn.onclick = () => {
        if (state.episodePage <= 1)
            return;
        state.episodePage -= 1;
        render();
    };
    nextEpisodePageBtn.onclick = () => {
        if (state.episodePage >= totalEpisodePages)
            return;
        state.episodePage += 1;
        render();
    };
